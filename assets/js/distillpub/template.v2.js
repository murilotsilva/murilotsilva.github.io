...
/* end bibtexParse */
  ...
  function normalizeTag(string) {
    var rxp = /\{(\\[^{}]+)\}/g, // Get TeX special characters within curly braces
        current_match;
    while (current_match = rxp.exec(string)) {
      if (latex_to_unicode[current_match[1]] == undefined){
        console.log(current_match[0] + "->" + current_match[1] + ": " + latex_to_unicode[current_match[1]]);
      };
      string = string.replace(current_match[0],latex_to_unicode[current_match[1]]);
    };
    rxp = /\{(\\.?\{[\\]?.?\}+)\}/g; // Get pure TeX accents within curly braces
    while (current_match = rxp.exec(string)) {
      if (latex_to_unicode[current_match[1]] == undefined){
        console.log(current_match[0] + "->" + current_match[1] + ": " + latex_to_unicode[current_match[1]]);
      };
